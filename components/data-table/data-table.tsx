...
interface DataTableProps<TData extends ExportableData, TValue> {
  ...
  // TanStack Query key for automatic data fetching & cache invalidation
  queryKey?: string[]
...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  // React Query mode (when queryKey is provided)
  const query = useQuery({
    queryKey: hasQueryKey
      ? [
          ...queryKey,
          page,
          pageSize,
    ...
    enabled: hasQueryKey,
    placeholderData: hasQueryKey ? keepPreviousData : undefined,
  })

  // Fallback: legacy useEffect mode (when no queryKey)
  const [legacyData, setLegacyData] = useState<DataFetchResult<TData> | null>(
    null
  )
  const [legacyLoading, setLegacyLoading] = useState(true)
  const [legacyError, setLegacyError] = useState(false)
  const [legacyErrorObj, setLegacyErrorObj] = useState<Error | null>(null)
  ...
  // Derive final state from either source
  const data = hasQueryKey ? (query.data ?? null) : legacyData
  const isLoading = hasQueryKey ? query.isLoading : legacyLoading
  const isError = hasQueryKey ? query.isError : legacyError
  const error = hasQueryKey ? (query.error ?? null) : legacyErrorObj
  ...
  const getSelectedItems = useCallback(async () => {
    ...
    const itemsInCurrentPage = dataItems.filter(
      (item: any) => selectedItemIds[String(item[idField])]
    )

    // Get IDs of items on current page
    const idsInCurrentPage = itemsInCurrentPage.map((item: any) =>
      String(item[idField])
    )
  ...
  const getSelectedParentRows = useCallback(async (): Promise<TData[]> => {
    ...
    const parentsInCurrentPage = dataItems.filter((item: any) =>
      selectedParentIds.has(String(item[idField]))
    )

    // Get IDs of parents on current page
    const idsInCurrentPage = new Set(
      parentsInCurrentPage.map((item: any) => String(item[idField]))
    )
  ...
  const handlePaginationChange = useCallback(
    (
      updaterOrValue:
        PaginationUpdater<TData> | { pageIndex: number; pageSize: number }
    ) => {
  ...
  const handleColumnOrderChange = useCallback(
    (updaterOrValue: ColumnOrderUpdater | string[]) => {
      ...
      const actionsIdx = newColumnOrder.indexOf("actions")
      if (actionsIdx > -1 && actionsIdx < newColumnOrder.length - 1) {
        newColumnOrder.splice(actionsIdx, 1)
    ...
    try {
      ...
      if (savedOrder) {
        ...
        if (
          ...
          // Ensure actions column is always last
          const actionsIdx = parsedOrder.indexOf("actions")
          if (actionsIdx > -1 && actionsIdx < parsedOrder.length - 1) {
            parsedOrder.splice(actionsIdx, 1)
            parsedOrder.push("actions")
          }
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className="table-container w-full overflow-x-auto overflow-y-auto rounded-xl border border-border/80 bg-card/35 shadow-sm backdrop-blur-sm transition-all duration-200"
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  const isSticky =
                    (header.column.columnDef.meta as any)?.sticky === "left"
                  const offset =
                    (header.column.columnDef.meta as any)?.stickyOffset ?? 0
                  return (
                    <TableHead
                      className={cn(
                        "group/th relative border-b bg-muted/40 px-4 py-3 text-left text-xs font-semibold tracking-wider text-muted-foreground/90 uppercase transition-colors select-none",
                        isSticky &&
                          "sticky left-0 z-30 bg-muted/95 shadow-[1px_0_0_0_rgba(0,0,0,0.1)] backdrop-blur-sm"
                      )}
                      key={header.id}
                      ...
                      style={{
                        ...
                        left: isSticky ? offset : undefined,
                      }}
            ...
            {isLoading ? (
              ...
              Array.from({ length: pageSize }).map((_, i) => (
                ...
                >
                  {Array.from({ length: columns.length }).map((_, j) => {
                    const col = columns[j]
                    const isSticky = (col as any)?.meta?.sticky === "left"
                    const offset = (col as any)?.meta?.stickyOffset ?? 0
                    return (
                      <TableCell
                        key={`skeleton-cell-${crypto.randomUUID()}`}
                        className={cn(
                          "max-w-0 truncate px-4 py-3 text-left",
                          isSticky &&
                            "sticky left-0 z-10 bg-background/95 shadow-[1px_0_0_0_rgba(0,0,0,0.05)]"
                        )}
                        tabIndex={-1}
                        style={{
                          left: isSticky ? offset : undefined,
                          ...
                          {subRowColumns.map((column, colIndex) => {
                            ...
                            return (
                              <TableCell
                                ...
                                className={cn(
                                  "relative h-9 border-b bg-muted/15 px-4 py-2 text-left align-middle text-xs font-semibold tracking-wider text-muted-foreground/80 uppercase",
                                  isSortable &&
                                    "cursor-pointer transition-colors select-none hover:bg-muted/25"
                                )}
                                style={{
                        ...
                        >
                          {subRowColumns.map((column, colIndex) => {
                            ...
                            const value = accessorKey
                              ...
                              : null
                            const isSticky =
                              (column as any)?.meta?.sticky === "left"
                            const offset =
                              (column as any)?.meta?.stickyOffset ?? 0
                            ...
                            return (
                              <TableCell
                                ...
                                className={cn(
                                  "max-w-0 truncate px-4 py-3 text-left",
                                  isSticky &&
                                    "sticky left-0 z-10 bg-background shadow-[1px_0_0_0_rgba(0,0,0,0.08)] group-hover:bg-muted/40 group-data-[state=selected]:bg-muted/50 dark:bg-zinc-950"
                                )}
                                style={{
                                  width: column.size || "auto",
                                  left: isSticky ? offset : undefined,
                  ...
                  return (
                    ...
                    >
                      {subRowColumns.map((column, colIndex) => {
                        ...
                        const isSticky =
                          (column as any)?.meta?.sticky === "left"
                        const offset = (column as any)?.meta?.stickyOffset ?? 0
                ...
                return (
                  ...
                  >
                    {row.getVisibleCells().map((cell, cellIndex) => {
                      const isSticky =
                        (cell.column.columnDef.meta as any)?.sticky === "left"
                      const offset =
                        (cell.column.columnDef.meta as any)?.stickyOffset ?? 0
                      return (
                        <TableCell
                          className={cn(
                            "max-w-0 truncate px-4 py-3 text-left transition-colors",
                            isSticky &&
                              "sticky left-0 z-10 bg-background shadow-[1px_0_0_0_rgba(0,0,0,0.08)] group-hover:bg-muted/40 group-data-[state=selected]:bg-muted/50 dark:bg-zinc-950"
                          )}
                          key={cell.id}
              ...
              <TableRow>
                <TableCell
                  ...
                  className="h-24 truncate px-4 text-left font-medium text-muted-foreground"
                >
                  No results found.
                </TableCell>
              </TableRow>
