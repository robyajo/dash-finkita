...
import { Home, ArrowLeft, ShieldBan } from "lucide-react"
...
export default function Page403() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-zinc-950 px-4 text-white">
      ...
      <div className="pointer-events-none absolute top-1/2 left-1/2 h-125 w-125 -translate-x-1/2 -translate-y-1/2 rounded-full bg-red-500/10 blur-[120px]" />
      <div className="pointer-events-none absolute top-1/3 left-1/3 h-75 w-75 -translate-x-1/2 -translate-y-1/2 rounded-full bg-amber-500/10 blur-[100px]" />

      <div className="relative z-10 flex max-w-md flex-col items-center space-y-6 text-center">
        {/* Glow icon */}
        <div className="flex h-20 w-20 items-center justify-center rounded-2xl border border-red-500/30 bg-red-500/10 text-red-400 shadow-[0_0_50px_rgba(239,68,68,0.1)]">
          <ShieldBan className="h-10 w-10 animate-bounce" />
        </div>
        ...
        <div className="flex w-full flex-col items-center gap-3 pt-2 sm:w-auto sm:flex-row">
          ...
          <Link href="/">
            <Button className="w-full cursor-pointer bg-indigo-600 text-white shadow-lg shadow-indigo-600/10 transition-all hover:bg-indigo-500 hover:shadow-indigo-600/25 sm:w-auto">
